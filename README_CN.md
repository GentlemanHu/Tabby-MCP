...
> ⚠️ **预发布版本，尚未发布到 npm。** 全部改动均已通过静态分析、类型检查、冒烟回归检查和生产构建。其中键盘交互式认证已在真实 Tabby 环境中完整跑通（从提交 MFA 到进入 Jumpserver 资产菜单，Tabby 1.0.229 与 1.0.235 / macOS）。其余修复——对话框焦点恢复、SFTP 传输的实际取消行为、多实例端口交接——**尚未**经过手动实机验证，仍需测试。可从 [GitHub 预发布页面](https://github.com/GentlemanHu/Tabby-MCP/releases/tag/v1.6.3-rc.1) 安装，如遇异常请在对应 issue 下反馈。
